...
- Filtering logic in `src/sdi/patterns/catalog.py`:
  - Compile `config.patterns.scope_exclude` to a glob matcher once at the start of `build_pattern_catalog` (`catalog.py:154-216`). Use `pathspec` — it is already a declared dependency (`pathspec>=0.11` in `pyproject.toml`), so no new top-level dependency is introduced. Use `pathspec.PathSpec.from_lines("gitwildmatch", config.patterns.scope_exclude)`.
...
[PM: Added — the rubric requires a Migration Impact section for milestones that add user-facing config keys or snapshot format changes.]

- **Config files:** `scope_exclude` defaults to `[]`. Existing `.sdi/config.toml` files without this key continue to work without change (Non-Negotiable Rule 12: missing keys fall through to built-in defaults).
- **Snapshot JSON:** The new `meta.scope_excluded_file_count` field is additive. Older snapshots that lack it deserialize correctly via `data.get("meta", {})`. No schema version bump is required — this is a backward-compatible addition within the same schema version.
- **`sdi diff` / `sdi trend`:** Snapshots produced before M17 compare cleanly against post-M17 snapshots. The absence of `meta` in the older snapshot is treated as `scope_excluded_file_count = 0`.
- **Config hash:** `_compute_config_hash` gains `scope_exclude` (sorted) in its input dict. A snapshot taken with `scope_exclude = []` and one taken before M17 (when the key did not exist) will hash identically because `sorted([]) == []` and the prior hash also omitted the key — document this edge case in the implementation notes. If the implementer chooses to always include the key in the hash (even when empty), accept that as a minor hash change: pre-M17 and post-M17 snapshots will show as config-hash-differing in `sdi diff`, which is accurate since the configuration format changed.
...
- `tests/integration/test_full_pipeline.py` — extend with:
  - new fixture `tests/fixtures/scope-exclude-python/` [PM: renamed from `scope-exclude-shell/` — shell/bash has no tree-sitter adapter in this project; files would be silently skipped at Stage 1 per Non-Negotiable Rule 10, making all pattern-count assertions vacuously true. Use Python files instead.] containing `lib/util.py` (one canonical error-handling pattern), `cmd/run.py` (one variant error-handling pattern), and `tests/scenario_a.py`, `tests/scenario_b.py`, `tests/scenario_c.py` (each using a distinct third/fourth/fifth error-handling variant — e.g., bare `except`, `except Exception`, `except Exception as e`, logging-based, and return-None-on-error styles).
