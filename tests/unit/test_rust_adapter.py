...
class TestModDeclarations:
    def test_external_mod_creates_import(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
        record = _parse(adapter, path, "mod submodule;\n")
        assert "./submodule" in record.imports

    def test_inline_mod_not_in_imports(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
    ...
    def test_multiple_external_mods(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
        source = "mod models;\nmod handlers;\n"
        record = _parse(adapter, path, source)
...
class TestPubItems:
    ...
    def test_private_function_excluded(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
...
class TestTraitAndImplBlocks:
    def test_impl_block_records_type(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
        source = "struct Foo; impl Foo { pub fn new() -> Self { Foo } }\n"
        record = _parse(adapter, path, source)
        assert "Foo" in record.symbols

    def test_trait_impl_records_self_type(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
        source = "pub trait MyTrait { fn do_it(&self); }\nstruct Bar;\nimpl MyTrait for Bar { fn do_it(&self) {} }\n"
        record = _parse(adapter, path, source)
...
class TestPatternInstances:
    def test_match_on_result_detected(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
        source = (
            'fn f() {\n    match do_thing() {\n        Ok(v) => v,\n        Err(e) => panic!("{}", e),\n    };\n}\n'
        )
    ...
    def test_pattern_has_required_keys(self, adapter: RustAdapter, repo_root: Path) -> None:
        path = repo_root / "lib.rs"
