...
def test_post_merge_hook_creates_executable_file(bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_pre_push_hook_creates_executable_file(bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_post_merge_hook_has_shebang(bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_post_merge_hook_contains_marker(bare_git_repo: Path, cli_runner: CliRunner) -> None:
    run_sdi(cli_runner, ["init", "--install-post-merge-hook"], bare_git_repo)
    hook = bare_git_repo / ".git" / "hooks" / "post-merge"
    assert POST_MERGE_MARKER in hook.read_text(encoding="utf-8")


def test_pre_push_hook_contains_marker(bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_install_appends_to_existing_hook(bare_git_repo: Path, cli_runner: CliRunner) -> None:
    hook = bare_git_repo / ".git" / "hooks" / "post-merge"
...
def test_install_does_not_duplicate_existing_block(bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_post_merge_hook_always_exits_zero(tmp_path: Path, bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_post_merge_hook_skips_non_main_branches(tmp_path: Path, bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_pre_push_hook_blocks_on_exit_10(tmp_path: Path, bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_pre_push_hook_allows_on_exit_0(tmp_path: Path, bare_git_repo: Path, cli_runner: CliRunner) -> None:
...
def test_post_merge_hook_valid_shell_syntax(bare_git_repo: Path, cli_runner: CliRunner) -> None:
    run_sdi(cli_runner, ["init", "--install-post-merge-hook"], bare_git_repo)
    hook = bare_git_repo / ".git" / "hooks" / "post-merge"
...
@pytest.mark.skipif(shutil.which("shellcheck") is None, reason="shellcheck not available")
def test_pre_push_hook_valid_shell_syntax(bare_git_repo: Path, cli_runner: CliRunner) -> None:
    run_sdi(cli_runner, ["init", "--install-pre-push-hook"], bare_git_repo)
    hook = bare_git_repo / ".git" / "hooks" / "pre-push"
    result = subprocess.run(["shellcheck", str(hook)], capture_output=True)
    assert result.returncode == 0, result.stderr.decode()
